...
import { getLeagueSnapshot, getScoreboard, teamName } from "@/lib/espn";
...
export default async function Home() {
  ...
  let matchups: Awaited<ReturnType<typeof getScoreboard>> | undefined;
  let loadError: string | null = null;

  try {
    snapshot = await getLeagueSnapshot();
    matchups = await getScoreboard(snapshot.currentWeek);
  } catch (err) {
    ...
    <div className="flex flex-col gap-10">
      <section>
        <p className="font-heading text-xs uppercase tracking-[0.14em] text-accent">
          {snapshot ? `Week ${snapshot.currentWeek}` : "League HQ"}
        </p>
        <h1 className="font-heading text-3xl sm:text-4xl font-bold mt-1 text-balance">
          {snapshot?.leagueName ?? "East End Gridiron Championship"}
        </h1>
      </section>

      {loadError && (
        <ErrorPanel
          title="Couldn't reach ESPN right now"
          message={`${loadError}. This usually means the espn_s2/SWID cookies need refreshing, or the season hasn't opened yet.`}
        />
      )}
      ...
      {matchups && matchups.length > 0 && snapshot && (
        <section>
